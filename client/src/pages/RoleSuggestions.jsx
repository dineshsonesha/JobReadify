...
export default function RoleSuggestions() {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await fetch("/api/ai/role-suggestions", {
        method: "POST",
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        <div className="flex items-center gap-3">
          <Lightbulb className="w-6 text-[#00DA83]" />
          <h1 className="text-xl font-semibold">Role Suggestions</h1>
        </div>
      ...
      <div className="w-full max-w-lg p-4 bg-white border border-gray-200 rounded-lg flex flex-col min-h-96 max-h-[600px]">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold">Suggested Roles</h1>
        </div>
        ...
        {!content ? (
          <div className="flex-1 flex justify-center items-center">
            <div className="text-sm flex flex-col items-center gap-5 text-gray-400">
              ...
              <p>
                Upload a resume and click "Get Role Suggestions" to get started
              </p>
            </div>
          </div>
        ) : (
